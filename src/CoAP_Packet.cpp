...
int CoAP_Packet::parse(uint8_t *buf, size_t buflen) {
    int rc;
    ...
    if (0 != (rc = parseToken(buf, buflen)))
        return rc;
    optionsNumber = COAP_MAX_OPTIONS_NUMBER;
...
int CoAP_Packet::parseHeader(uint8_t *buf, size_t buflen) {
    if (buflen < 4)
...
int CoAP_Packet::parseToken(uint8_t *buf, size_t buflen) {
    if (header.tkl == 0) {
        token.p = NULL;
        token.len = 0;
        return 0;
...
int CoAP_Packet::parseOption(PacketOption *option, uint16_t *runningDelta, uint8_t **buf, size_t buflen) {
    uint8_t *p = *buf;
    uint8_t headlen = 1;
    uint16_t len, delta;
...
int CoAP_Packet::parseOptionsAndPayload(uint8_t *buf, size_t buflen) {
    size_t optionIndex = 0;
    uint16_t delta = 0;
    uint8_t *p = buf + 4 + header.tkl;
